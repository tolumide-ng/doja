...
impl Magic {
    ...
    pub(crate) fn find_magic(&mut self, sq: u64, relevant_bits: u32, bishop: bool) -> u64 {
        ...
        for i in 0..(1<<n) {
            ...
            attacks[i] = match bishop {
                true => PIECE_ATTACKS.get_bishop_attacks_on_the_fly(sq, b[i]).into(),
                false => PIECE_ATTACKS.get_rook_attacks_on_the_fly(sq, b[i]).into(),
            };
        }
    ...
    pub(crate) fn finding_finder(&mut self) {
        ...
        for sq in 0..64 {
            ...
            bishop.push(self.find_magic_number(sq, BISHOP_RELEVANT_BITS[sq as usize], true));
        }
    ...
    pub(crate) fn find_magic_number(&mut self, sq: u64, relevant_bits: u32, bishop: bool) -> u64 {
        ...
        for i in 0..occupancy_indices {
            ...
            let indexed_attacks = match bishop {
                ...
                false => PIECE_ATTACKS.get_rook_attacks_on_the_fly(sq, occupancies[index])
            };
